        ...
        <div className="border-2 border-black/20 p-3 mb-6 boot-up hidden md:block">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-4">
              <span className="text-[10px] text-black/50 uppercase tracking-wider">PRODUCT_DETAIL</span>
              <span className="text-[10px] text-red-500">●VIEWING</span>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-3 text-[10px] text-black/50 uppercase tracking-wider">
              <span>IMG_{selectedImage + 1}/{allImages.length}</span>
            ...
            <div className="relative aspect-[3/4] mb-4 overflow-hidden bg-white border-2 border-black/20 scan-effect bracket-container">
              ...
              <div className="bracket top-left" style={{ width: '16px', height: '16px' }} />
              <div className="bracket top-right" style={{ width: '16px', height: '16px' }} />
              <div className="bracket bottom-left" style={{ width: '16px', height: '16px' }} />
              <div className="bracket bottom-right" style={{ width: '16px', height: '16px' }} />
          ...
          <div className="flex flex-col">
            ...
            <div className="border-2 border-black/20 p-4 mb-6 reveal-left">
              <div className="text-[10px] text-black/50 uppercase tracking-wider mb-2">PRODUCT_DESIGNATION</div>
              <h1 className="uppercase tracking-wide text-black mb-2">{product.name}</h1>
            ...
            {product.sizes && (
              <div className="border-2 border-black/20 p-4 mb-6">
                <div className="text-[10px] text-black/50 uppercase tracking-wider mb-3">SELECT_SIZE</div>
            ...
            {product.variants && (
              <div className="border-2 border-black/20 p-4 mb-6">
                <div className="text-[10px] text-black/50 uppercase tracking-wider mb-3">SELECT_OPTION</div>
                <div className="space-y-2">
                  {product.variants.map((variant) => {
                    const variantDiscount = Math.round(((variant.originalPrice - variant.price) / variant.originalPrice) * 100);
                    return (
                      <button
                        key={variant.id}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="uppercase tracking-wide text-black">{variant.name}</span>
                          <div className="flex items-center gap-2">
                            {variantDiscount > 0 && (
                              <span className="text-black/40 price-slash text-sm">${variant.originalPrice.toFixed(2)}</span>
                            )}
                            <span className="text-red-500 tracking-wider">${variant.price.toFixed(2)}</span>
                          </div>
                        </div>
                        {variantDiscount > 0 && (
            ...
            {!product.variants && (
              <div className="border-2 border-red-500 bg-red-500/5 p-4 mb-6">
                <div className="text-[10px] text-black/50 uppercase tracking-wider mb-3">PRICING_DATA</div>
                <div className="flex items-baseline gap-4 mb-2">
                  <div>
                    <div className="text-[8px] text-black/30 uppercase tracking-wider mb-1">ORIGINAL</div>
                    <span className="text-black/40 price-slash">${currentOriginalPrice.toFixed(2)}</span>
                  </div>
                  <div>
                    <div className="text-[8px] text-black/30 uppercase tracking-wider mb-1">CURRENT</div>
                    <span className="text-red-500 text-2xl price-fade-in tracking-wider">${currentPrice.toFixed(2)}</span>
                  </div>
                </div>
                {discount > 0 && (
            ...
            <div className="border border-black/20 p-4 mb-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] text-black/50 uppercase tracking-wider">DESCRIPTION</span>
                <div className="h-px flex-1 bg-black/10" />
              </div>
              <p className="text-black/70 text-sm leading-relaxed">
                {product.longDescription || 'Premium quality merchandise from Pretty Tony\'s Autoshop. Each item is carefully selected to represent our brand and passion for automotive excellence.'}
              </p>
            </div>
            ...
            <div className="border border-black/20 p-4 mb-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] text-black/50 uppercase tracking-wider">SPECIFICATIONS</span>
                <div className="h-px flex-1 bg-black/10" />
              </div>
              <div className="space-y-2">
                <div className="h-px bg-black/10" />
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-red-500">●LIMITED_QUANTITY</span>
                </div>
              </div>
            </div>
            ...
            {/* Quantity Selection */}
            <div className="border-2 border-black/20 p-4 mb-6">
              <div className="text-[10px] text-black/50 uppercase tracking-wider mb-3">QUANTITY</div>
              <div className="flex items-center gap-4">
                <button
                ...
                <div className="flex-1 border-2 border-black/20 p-3 text-center">
                  <span className="text-2xl tracking-wider text-black">{quantity}</span>
                </div>
                <button
            ...
            <div className="mt-auto">
              <button
                onClick={() => onAddToCart(product.id, quantity, selectedVariant?.id || selectedSize || undefined)}
